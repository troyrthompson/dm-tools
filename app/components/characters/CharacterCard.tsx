  ...
  return (
    <div className="min-w-1/12 min-w-36 block w-full-sm bg-white border border-gray-200 rounded shadow dark:bg-gray-800 dark:border-gray-700 no-underline">
        ...
        <Link className="text-center block mb-2" href={`./character/?id=${character.id}`} key={character.id}><h5 className="text-xl leading-none font-medium text-gray-900 dark:text-white px-4">{character.general.name}</h5>
            ...
            <span onBlur={handleNotesBlur} suppressContentEditableWarning="true" contentEditable className="text-xs align-item-end border border-gray-300 text-gray-400 w-28 resize-none mt-2">{character.notes}</span>
        </div>
    </div>    
  );
};
